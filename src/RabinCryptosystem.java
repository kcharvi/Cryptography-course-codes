...
class Cryptography {
	...
	public static BigInteger encrypt(BigInteger m,BigInteger N){
		return m.modPow(TWO, N);  //C=Message^2 mod N
	}
...
public class RabinCryptosystem {
	public static void main(String[] args)
	{
		...
		System.out.println("Encryption at sender : " + c);
		BigInteger[] m2 = Cryptography.decrypt(c, p, q);
		for (BigInteger b : m2) {
			String dec = new String(
				b.toByteArray(),
				Charset.forName("ascii"));
			if (dec.equals(s))finalMessage = dec;
		}
		System.out.println("Deccryption at receiver : " + finalMessage);
	}
}
